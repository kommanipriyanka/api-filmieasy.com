...
const nonBlankName = (fieldName: string) =>
  v.pipe(
...
export const vArtistObject = v.object({
  ...
  amount: v.optional(v.pipe(v.number(),
      v.check(n => Number.isInteger(n), "Amount must be an integer"),
      v.check(n => n >= 0, "Amount cannot be negative"),
    ),
  ),
});
...
export const vArtistSchema = v.pipe(
  ...
  v.check((d: any) =>!!(!d.payment_type|| d.payment_type !== "BANK"|| d.payment_details === undefined|| d.payment_details === null|| (d.payment_details && d.payment_details.account_number)),
    "Account_number is required for BANK payment type",
  ),
  v.check(
    (d: any) =>
  ...
  v.check((d: any) =>!!(!d.payment_type|| d.payment_type !== "BANK"|| d.payment_details === undefined|| d.payment_details === null|| (d.payment_details && d.payment_details.name)),
  "Name is required for BANK payment type",
  ),
  v.check((d: any) =>!!(!d.payment_type|| d.payment_type !== "BANK"|| d.payment_details === undefined|| d.payment_details === null|| (d.payment_details && d.payment_details.branch_name)),
  "Branch_name is required for BANK payment type",
  ),
  v.check((d: any) =>
    !!(
      !d.payment_type
...
v.check(
  ...
  v.transform(obj => {
    const out = { ...obj };
    if (out.payment_type === "CASH") out.payment_details = null;
    return out;
  }),
);
