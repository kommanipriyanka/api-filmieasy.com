...
import type { DBRecord, DBTable, DBTableColumns, InQueryData, OrderByQueryData, SortDirection, WhereQueryData } from "../types/dbTypes";
      ...
      case "OR":
        if (Array.isArray(value) && value.length > 0) {
          ...
          for (const cond of value) {
            const col = table[cond.column as keyof typeof table] as unknown as SQLWrapper;
            switch (cond.relation) {
              case "=":
                orGroup.push(sql`${col} = ${cond.value}`);
                break;
              case "IN":
                orGroup.push(sql`${col} IN (${sql.join(cond.value, sql`, `)})`);
                break;
              case "IS NULL":
                orGroup.push(isNull(col));
                break;
              case "IS NOT NULL":
                orGroup.push(isNotNull(col));
                break;
            }
          }
          if (orGroup.length > 0) {
            whereQueries.push(sql`(${sql.join(orGroup, sql` OR `)})`);
          }
        }
        break;
