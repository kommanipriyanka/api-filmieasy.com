...
import { saveRecords } from "./baseDbServices";
...
export class UserService{
  ...
  return allArtists.map(artist => ({...artist,
    DOB: artist.DOB? new Date(artist.DOB).toLocaleDateString("en-US", {month: "short",day: "numeric",year: "numeric",}): null,}));
  }
 ...
 importArtistsService = async (records: any[]) => {
  if (!records?.length) return { insertedRecords: 0, skippedDuplicates: 0 };
